...
ShapeBase=[
    ...
    ('Ellipse',100,40),
    ('Rectangle',80,40),
    ('Polygon',60,0,0,120,-60,0,-30,0,-30,-60,30,-60,30,0),
...
class MyCanvas(QtGui.QWidget):
    ...
    def PolygonGenerator(self,*points):
        r=np.asarray(list(points[0::2]))
        c=np.asarray(list(points[1::2]))
        x,y=skimage.draw.polygon(r,c)
        
        if min(x)<=0 or max(x)>=Width or min(y)<=0 or max(y)>=Height:
            return False
        else:
            # print min(x),max(x),min(y),max(y)
            for i in range(0,len(x)):
                if pixelmat[x[i]][y[i]]!=0:
                    return False
            pixelmat[x,y]=1
